...
<body class="bg-slate-50">
        ...
        <div class="space-y-3">
            ...
            <div id="filter-inputs" class="flex flex-col gap-2">
                <div id="input-single" class="hidden">
                    <input type="date" id="date-single" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-4 text-[10px] font-black uppercase text-blue-400">
                </div>
                <div id="input-range" class="hidden grid grid-cols-2 gap-2">
                    <input type="date" id="date-start" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-4 text-[10px] font-black uppercase text-blue-400">
                    <input type="date" id="date-end" class="w-full bg-slate-800 border border-slate-700 rounded-xl py-2 px-4 text-[10px] font-black uppercase text-blue-400">
                </div>
                <button onclick="updateHistory()" class="w-full bg-blue-600 hover:bg-blue-700 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg transition-all active:scale-95">
    ...
    <div class="bg-white border-t border-slate-100 shrink-0 shadow-[0_-15px_35px_rgba(0,0,0,0.1)] z-20 overflow-y-auto max-h-[45vh] no-scrollbar">
                ...
                <table class="min-w-full divide-y divide-slate-100 report-table">
                    <thead class="bg-slate-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-[9px] font-black text-slate-400 uppercase">Lokasi</th>
                        </tr>
                    </thead>
    ...
    <script>
        ...
        function updateHistory() {
            ...
            if (mode === 'today') {
                ...
                params = `date=${document.getElementById('date-single').value}`;
            } else if (mode === 'range') {
                params = `start=${document.getElementById('date-start').value}&end=${document.getElementById('date-end').value}`;
            }

            loadHistory(params);
        }

        function loadHistory(queryString) {
            if (pathLine) map.removeLayer(pathLine);
            markers.forEach(m => map.removeLayer(m));
            markers = [];
            
            const url = `/api/history/{{ $device->imei }}?${queryString}`;
            document.getElementById('print-date').innerText = queryString.split('=')[1] || "Periode Terpilih";
            ...
            fetch(url)
                ...
                .then(data => {
                    ...
                    if (!data || data.length === 0) {
                        parkingTable.innerHTML = '<tr><td colspan="3" class="p-8 text-center text-slate-300 italic text-xs">Data tidak ditemukan.</td></tr>';
                        return;
                    }
                    ...
                    data.forEach((p) => {
                        ...
                        if (lastP) {
                            ...
                            if (timeDiff > 300000) { // > 5 menit
                                parkingEvents.push({ lat: lastP.latitude, lng: lastP.longitude, startTime: lastP.gps_time, duration: timeDiff });
                            }
                    ...
                    pathLine = L.polyline(filteredPoints, { color: '#3b82f6', weight: 5, opacity: 0.7 }).addTo(map);
                    
                    parkingEvents.forEach((evt, i) => {
                        const googleUrl = `https://www.google.com/maps?q=${evt.lat},${evt.lng}`;
                        parkingTable.innerHTML += `
                            <tr>
                                <td class="px-4 py-3 text-[10px] font-bold text-slate-700">${new Date(evt.startTime.replace(' ', 'T') + 'Z').toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'})}</td>
                                <td class="px-4 py-3 text-[10px] font-black text-amber-500 uppercase">${Math.floor(evt.duration/60000)} mnt</td>
                                <td class="px-4 py-3">
                                    <a href="${googleUrl}" target="_blank" class="text-[9px] font-mono text-blue-500 flex items-center gap-1">
                                        <i class="fa-solid fa-location-dot"></i> Lihat Map
                                    </a>
                                </td>
                            </tr>
                        `;
